...
def load_embeddings(embeddings_path: str) -> np.ndarray:
    """Load precomputed review embeddings from a .npy file."""
...
def get_top_k_reviews(
    ...
    """
    if len(reviews_df) != len(review_embeddings):
        raise ValueError(
            f"Length mismatch: reviews_df has {len(reviews_df)} rows, "
            f"but review_embeddings has {len(review_embeddings)} rows."
        )

    scores = cosine_similarity(query_embedding, review_embeddings)[0]  # shape: (n_reviews,)
...
def aggregate_to_restaurants(
    ...
    Returns top_n restaurants ranked by average similarity score.
    """
    if "gmap_id" not in top_reviews.columns:
        raise KeyError("top_reviews must contain a 'gmap_id' column.")
    ...
    # only return the existing colums
    existing_cols = [col for col in output_cols if col in results.columns]
